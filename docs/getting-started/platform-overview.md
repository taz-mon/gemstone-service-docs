...
**When GemStone fits:** GemStone works well for applications with deeply nested or highly interconnected data structures, where the cost of translating objects to and from relational tables creates performance or complexity problems. It also works well for workloads where business logic runs close to the data, inside the server or in GemStone Smalltalk, not in a separate application tier.
...
The most significant architectural difference is that GemStone eliminates the object-relational impedance mismatch entirely. The application models data in whatever structure it requires, including hierarchies, networks, queues, and nested collections. GemStone does not assemble data objects at query time.
...
You establish persistence by attaching a new object to an already-persistent object and committing the transaction. Until that commit, the object exists only in the session's private memory and other users cannot see it. After the commit, the object becomes part of the shared repository and users in authorized sessions can see it.

This is different from a relational database, where an `INSERT` immediately targets shared, durable storage. In GemStone, you work with objects freely in session memory, attach them to the persistent graph when they are ready, and commit when the transaction is complete. Objects that never attach to the persistent graph are temporary; GemStone discards them when the session ends.

A single user may have multiple simultaneous sessions open, and each session maintains its own consistent snapshot view of the repository. Changes from other sessions become visible after the next transaction boundary.
...
- **GemStone Smalltalk / Topaz** — the native command-line interface for repository operations, scripting, and development. GemTalk includes Topaz with the server distribution..
- **GemBuilder for C** — a library of C functions that provides a bridge between C application code and the GemStone repository. GemTalk includes GemBuilder for C with the server distribution.
